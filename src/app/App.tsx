...
export default function App() {
  ...
  const [showStartDrawingReminder, setShowStartDrawingReminder] = useState(false);
  ...
  function selectPointLocation(latitude: number, longitude: number) { if (!planId) { setMessage("[警告] 請先建立勤務計畫，再新增崗哨。"); return; } if (activeNav === "路線" && isDrawingRoute) { setManualVertices((current) => [...current, [longitude, latitude]]); setMessage("已加入路線節點；完成後請點選「完成繪製」。"); return; } if (activeNav === "路線") { setMessage(manualVertices.length >= 2 ? "請輸入路線名稱保存路線。" : "尚未點選「開始繪製」。"); return; } if (activeNav === "部署表") { setSelectedPointId(null); setSelectedPersonnelLabelPointId(null); setMessage("已取消部署表崗哨選取。 "); return; } if (activeNav !== "點位") { setMessage("請先從頂部導覽選擇「崗哨」，再於地圖新增勤務崗哨。"); return; } if (!isPlacingPoint) { setShowStartDrawingReminder(true); return; } if (selectedPointId) { setSelectedPointId(null); setMessage("已取消選取勤務崗哨。"); return; } setPendingCoordinate({ latitude, longitude, pointType: "duty", color: pointColor }); setMessage("已選取底圖位置，請填寫崗哨編號與名稱後保存。"); }
  function selectCustomPointLocation(x: number, y: number) { if (!planId) return; if (activeNav === "路線" && isDrawingRoute) { setManualVertices((current) => [...current, [x, y]]); setMessage("已加入 XY 路線節點；完成後請點選「完成繪製」。"); return; } if (activeNav !== "點位") { setMessage("請先選擇「崗哨」或「路線繪製」。"); return; } if (!isPlacingPoint) { setShowStartDrawingReminder(true); return; } if (selectedPointId) { setSelectedPointId(null); return; } setPendingCoordinate({ latitude: y, longitude: x, coordinateX: x, coordinateY: y, pointType: "duty", color: pointColor }); setMessage("已選取底圖位置；請填寫崗哨編號與名稱後保存。"); }
  ...
  return (
    <main className={`app-shell ${activeNav === "地圖輸出" ? "map-output-mode" : activeNav === "部署表" ? "preview-mode" : ""} ${activePlan?.planMode === "custom_basemap" ? "custom-basemap-mode" : ""}`}>
      ...
      {showStartDrawingReminder && <div className="confirm-backdrop" role="dialog" aria-modal="true" aria-labelledby="start-drawing-title"><section className="confirm-dialog"><h2 id="start-drawing-title">請先開始繪製</h2><p>請先點選地圖右下角的「開始繪製」，再點擊底圖新增崗哨。</p><div><button type="button" onClick={() => setShowStartDrawingReminder(false)}>知道了</button></div></section></div>}
