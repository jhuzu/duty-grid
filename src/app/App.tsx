...
type ManualDeploymentRow = { id: string; deploymentScope: string; postType: string; location: string; unit: string; radioCode: string; coordinatorPhone: string; personnelIds: string[]; equipment: string[] };
...
export default function App() {
  ...
  const manualDeploymentRef = useRef<HTMLElement | null>(null);
  ...
  const [showClearPersonnelWarning, setShowClearPersonnelWarning] = useState(false);
  ...
  const [manualDeploymentTarget, setManualDeploymentTarget] = useState("all");
  const [manualDeploymentScrollRequest, setManualDeploymentScrollRequest] = useState(0);
  ...
  async function savePointDetails(point: MapDutyPoint, form: HTMLFormElement) {
    ...
    try {
      const updated = await invoke<MapDutyPoint>("update_duty_point", { pointId: point.id, input: { pointCode: String(values.get("pointCode") ?? ""), pointName: String(values.get("pointName") ?? ""), note: String(values.get("note") ?? ""), color: String(values.get("color") ?? ""), pointType: point.pointType, latitude, longitude, coordinateX: isCustomBasemap ? point.coordinateX ?? point.longitude : undefined, coordinateY: isCustomBasemap ? point.coordinateY ?? point.latitude : undefined } });
  ...
  async function clearPersonnel() { try { await invoke("clear_personnel"); setPersonnel([]); setPersonnelAssignments([]); setPersonnelImportLog(null); setPersonnelKeyword(""); setPersonnelUnit(""); setPersonnelTitle(""); setShowPersonnelLabels(false); setManualDeploymentRows((rows) => rows.map((row) => ({ ...row, personnelIds: [] }))); setMergedManualDeploymentRows((rows) => rows.map((row) => ({ ...row, personnelIds: [] }))); setShowClearPersonnelWarning(false); setMessage("已清除所有人員資料與人力配置，回到未有人員狀態。"); } catch (error) { setMessage(`無法清除人力資料：${String(error)}`); } }
  ...
  const baseDeploymentPoints = deploymentRoute ? deploymentRoute.pointIds.length ? deploymentRoute.pointIds.map((id) => points.find((point) => point.id === id)).filter((point): point is MapDutyPoint => Boolean(point)) : points.filter((point) => isPointNearRoute(point, routeCoordinates(deploymentRoute))) : points;
  ...
  const deploymentFocusCoordinates = activeNav === "部署表" && !deploymentRoute ? deploymentPoints.map((point) => activePlan?.planMode === "custom_basemap" ? [point.coordinateX ?? point.longitude, point.coordinateY ?? point.latitude] as [number, number] : [point.longitude, point.latitude] as [number, number]) : undefined;
  ...
  const activeDeploymentScope = deploymentRoute?.id ?? "all";
  const deploymentScopeLabel = (scope: string) => scope === "all" ? "全部崗位" : routes.find((route) => route.id === scope)?.routeName ?? "已刪除路線";
  const deploymentRows: DeploymentTableRow[] = [...pointDeploymentRows, ...mergedManualDeploymentRows.filter((manualRow) => manualRow.deploymentScope === activeDeploymentScope).map((manualRow, index) => {
  ...
  function openManualDeployment() { setManualDeploymentTarget(activeDeploymentScope); setManualDeploymentOpen(true); setManualDeploymentScrollRequest((value) => value + 1); }
  function addManualDeploymentRow() { setManualDeploymentRows((rows) => [...rows, { id: crypto.randomUUID(), deploymentScope: manualDeploymentTarget, postType: "", location: "", unit: "", radioCode: "", coordinatorPhone: "", personnelIds: [], equipment: [] }]); setManualDeploymentScrollRequest((value) => value + 1); }
  ...
  return (
    <main className={`app-shell ${activeNav === "地圖輸出" ? "map-output-mode" : activeNav === "部署表" ? "preview-mode" : ""} ${activePlan?.planMode === "custom_basemap" ? "custom-basemap-mode" : ""}`}>
      ...
      <section className="workspace" aria-label="地圖工作區">
        ...
        {(activeNav === "人力配置" || activeNav === "部署表") && <div className="map-personnel-actions"><button type="button" onClick={() => setShowPersonnelLabels((value) => !value)}>{showPersonnelLabels ? "隱藏警力" : "顯示警力"}</button></div>}
      ...
      <aside className={activeNav === "路線" ? "inspector route-inspector" : activeNav === "人力配置" ? "inspector personnel-inspector" : "inspector"} aria-label="詳細資料面板" onClick={(event) => { const target = event.target as HTMLElement; if (activeNav === "點位" && !target.closest("button, input, select, textarea, label")) { setSelectedPointId(null); setEditingPointId(null); return; } if (event.target === event.currentTarget) { setSelectedPointId(null); setSelectedRouteId(null); } }}>
        <h1>{activeNav === "點位" ? "勤務崗哨" : activeNav === "路線" ? "勤務路線" : activeNav === "人力配置" ? "警力配置" : activeNav === "部署表" ? "安全維護部署表預覽" : activeNav === "地圖輸出" ? "地圖輸出預覽" : activeNav === "路線調整（未實作）" ? "路線調整（未實作）" : planId ? "勤務計畫" : "開始建立勤務計畫"}</h1>
        ...
        {activeNav === "部署表" && <section className="deployment-preview">
          ...
          <div className="deployment-route-tabs" role="tablist" aria-label="勤務路線部署表">
            <button aria-selected={!deploymentRouteId} className={!deploymentRouteId ? "deployment-route-tab active" : "deployment-route-tab"} role="tab" type="button" onClick={() => { setDeploymentRouteId(null); setSelectedPointId(points[0]?.id ?? null); setSelectedPersonnelLabelPointId(null); setEquipmentEditorPointId(null); setMessage("已切換為全部崗位；可直接編製部署表。 "); }}>全部崗位</button>{routes.map((route) => <button aria-selected={deploymentRouteId === route.id} className={deploymentRouteId === route.id ? "deployment-route-tab active" : "deployment-route-tab"} key={route.id} role="tab" type="button" onClick={() => { setDeploymentRouteId(route.id); setSelectedPointId(route.pointIds[0] ?? null); setSelectedPersonnelLabelPointId(null); setEquipmentEditorPointId(null); setMessage(`已定位路線「${route.routeName}」的第一點位。`); }} onDoubleClick={() => void renameRoute(route)} title="雙擊可改名">{route.routeName}</button>)}<span className="deployment-manual-hint">↓ 點擊按鈕以手動新增</span>
          </div>
          <section className="deployment-workspace-note"><p>{deploymentRoute ? `目前檢視「${deploymentRoute.routeName}」：地圖與部署表僅顯示此路線的點位。點選表格列可定位地圖點位。` : "目前檢視全部崗位；不必先建立路線，也可直接配置人力及匯出部署表。"}</p><div className="deployment-inline-actions">{deploymentRoute && <div className="deployment-inline-import"><label><span>匯入部署表</span><select aria-label="匯入部署表" value={deploymentPointToAddId} onChange={(event) => setDeploymentPointToAddId(event.target.value)}><option value="">請選擇點位</option>{points.filter((point) => !deploymentPoints.some((item) => item.id === point.id)).map((point) => <option key={point.id} value={point.id}>{point.pointCode}｜{point.pointName}</option>)}</select></label><button type="button" disabled={!deploymentPointToAddId} onClick={addPointToDeploymentTable}>加入</button></div>}<button className="manual-deployment-trigger" type="button" onClick={openManualDeployment}>手動新增崗哨</button></div></section>
      ...
      <footer className="status-bar"><span>資料庫：尚未初始化　｜　路口參考：已隨 App 提供</span><span className="map-coordinate">座標：{mapCoordinate ? `經度 ${mapCoordinate.longitude.toFixed(6)}，緯度 ${mapCoordinate.latitude.toFixed(6)}` : "將游標移到地圖上顯示經緯度"}</span><span>© Jhexu. All rights reserved</span></footer>
      ...
      {showClearPersonnelWarning && <div className="confirm-backdrop" role="dialog" aria-modal="true" aria-labelledby="clear-personnel-title"><section className="confirm-dialog"><h2 id="clear-personnel-title">清除所有人員資料？</h2><p>將清除目前的人員資料、所有勤務的人力配置與最近匯入紀錄，並回到未有人員狀態。</p><p>此動作無法復原，確定繼續？</p><div><button type="button" onClick={() => setShowClearPersonnelWarning(false)}>取消</button><button className="delete-button" type="button" onClick={() => void clearPersonnel()}>確認清除</button></div></section></div>}
