...
export default function App() {
  ...
  const mapOutputPoliceRows = deploymentRows.map((row) => ({ postType: row.postType || "未設定", icon: row.point.pointType === "signal" ? "🚦" : row.point.pointType === "hollow" ? "○" : "●", count: row.count }));
  ...
  const manualEquipmentEditorRow = manualDeploymentRows.find((row) => row.id === manualEquipmentEditorRowId);
          ...
          <div className="a4-preview" onClick={(event) => { if (event.target === event.currentTarget) { setSelectedPointId(null); setSelectedPersonnelLabelPointId(null); setEquipmentEditorPointId(null); } }}>
            <div className="deployment-sheet">
              ...
              <table><thead><tr><th>編號</th><th>崗哨別</th><th>崗哨位置</th><th>派遣單位</th><th>警力</th><th>職稱姓名</th><th>無線電代號</th><th>服裝及應勤裝備</th><th>分（協調）區協調員電話</th></tr></thead><tbody>{deploymentRows.map((row) => {
                ...
                }}><option value="">請選擇</option>{deploymentUnits.map((value) => <option key={value} value={value}>{value}</option>)}</select></td><td>{row.count}</td><td>{row.names}</td><td>{row.radios}</td><td className="equipment-cell">{row.manualRow ? (row.equipment.join("、") || "—") : <button type="button" onClick={(event) => { event.stopPropagation(); setSelectedPointId(row.point.id); setShowPersonnelLabels(false); setSelectedPersonnelLabelPointId(row.point.id); setEquipmentEditorPointId(row.point.id); }}>{row.equipment.length ? row.equipment.join("、") : "選擇裝備"}</button>}</td><td>—</td></tr>;
              })}</tbody></table>
            </div>
          </div>
