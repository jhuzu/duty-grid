...
type ManualDeploymentRow = { id: string; postType: string; location: string; unit: string; radioCode: string; personnelIds: string[]; equipment: string[] };
...
type WorkspaceState = { planId: string; activeNav: string; selectedPointId: string | null; selectedRouteId: string | null; deploymentRouteId: string | null; deploymentChoices: Record<string, { postType: string; unit: string; radioCode?: string }>; mapOutputTitle: string; mapOutputZoom: number; mapOutputBearing: number };
...
export default function App() {
  ...
  const [deploymentChoices, setDeploymentChoices] = useState<Record<string, { postType: string; unit: string; radioCode?: string }>>({});
  ...
  function addManualDeploymentRow() { setManualDeploymentRows((rows) => [...rows, { id: crypto.randomUUID(), postType: "", location: "", unit: "", radioCode: "", personnelIds: [], equipment: [] }]); }
            ...
            <div className="deployment-sheet">
              ...
              <table><thead><tr><th>編號</th><th>崗哨別</th><th>崗哨位置</th><th>派遣單位</th><th>警力</th><th>職稱姓名</th><th>無線電代號</th><th>服裝及應勤裝備</th><th>分（協調）區協調員電話</th></tr></thead><tbody>{deploymentRows.map((row) => {
                ...
                return <tr className={!isManual && selectedPointId === row.point.id ? "selected" : ""} key={row.point.id} onClick={() => {
                  ...
                  else setDeploymentChoices((current) => ({ ...current, [row.point.id]: { postType: event.target.value, unit: current[row.point.id]?.unit ?? row.units, radioCode: current[row.point.id]?.radioCode } }));
                  ...
                  else setDeploymentChoices((current) => ({ ...current, [row.point.id]: { postType: current[row.point.id]?.postType ?? row.postType, unit: event.target.value, radioCode: current[row.point.id]?.radioCode } }));
                }}><option value="">請選擇</option>{deploymentUnits.map((value) => <option key={value} value={value}>{value}</option>)}</select></td><td>{row.count}</td><td>{row.names}</td><td><input aria-label="無線電代號" value={row.radios === "—" ? "" : row.radios} onClick={(event) => event.stopPropagation()} onChange={(event) => { if (row.manualRow) updateMergedManualDeploymentRow(row.manualRow.id, { radioCode: event.target.value }); else setDeploymentChoices((current) => ({ ...current, [row.point.id]: { postType: current[row.point.id]?.postType ?? row.postType, unit: current[row.point.id]?.unit ?? row.units, radioCode: event.target.value } })); }} /></td><td className="equipment-cell">{row.manualRow ? <button type="button" onClick={(event) => { event.stopPropagation(); setManualEquipmentEditorRowId(row.manualRow!.id); }}>{row.equipment.length ? row.equipment.join("、") : "選擇裝備"}</button> : <button type="button" onClick={(event) => { event.stopPropagation(); setSelectedPointId(row.point.id); setShowPersonnelLabels(false); setSelectedPersonnelLabelPointId(row.point.id); setEquipmentEditorPointId(row.point.id); }}>{row.equipment.length ? row.equipment.join("、") : "選擇裝備"}</button>}</td><td>—</td></tr>;
              })}</tbody></table>
            </div>
          </div>
