...
type WorkspaceState = { planId: string; activeNav: string; selectedPointId: string | null; selectedRouteId: string | null; deploymentChoices: Record<string, { postType: string; unit: string }>; mapOutputTitle: string; mapOutputZoom: number };
...
export default function App() {
  ...
  useEffect(() => { if (activeNav === "地圖輸出") { setMapOutputZoom(0); setMapOutputBearing(90); } }, [activeNav]);
  useEffect(() => {
    if (!planId) return;
  ...
  function selectPointLocation(latitude: number, longitude: number) { if (!planId) { setMessage("請先建立勤務計畫，再新增點位。"); return; } if (isDrawingRoute) { setManualVertices((current) => [...current, [longitude, latitude]]); setMessage("已加入路線節點；按 Enter 完成，Backspace 復原。 "); return; } if (activeNav === "部署表") { setSelectedPointId(null); setSelectedPersonnelLabelPointId(null); setMessage("已取消部署表點位選取。 "); return; } if (activeNav !== "點位") { setMessage("請先從頂部導覽選擇「點位」，再於地圖新增勤務點位。"); return; } if (selectedPointId) { setSelectedPointId(null); setMessage("已取消選取勤務點位。"); return; } setPendingCoordinate({ latitude, longitude }); setMessage("已選取地圖位置；請填寫點位編號與名稱後保存。"); }
  ...
  async function updateRouteLineStyle(lineStyle: "solid" | "dashed") { setRouteLineStyle(lineStyle); const route = routes.find((item) => item.id === selectedRouteId); if (!route) return; const previousLineStyle = route.lineStyle ?? "solid"; setRoutes((current) => current.map((item) => item.id === route.id ? { ...item, lineStyle } : item)); try { await invoke("update_duty_route_line_style", { routeId: route.id, lineStyle }); setMessage(`已更新路線 ${route.routeName} 為${lineStyle === "solid" ? "實線" : "虛線"}。`); } catch (error) { setRoutes((current) => current.map((item) => item.id === route.id ? { ...item, lineStyle: previousLineStyle } : item)); setRouteLineStyle(previousLineStyle); setMessage(`無法更新路線樣式：${String(error)}`); } }
  ...
  async function saveWorkspace() {
    ...
    try {
      await invoke("save_workspace_state", { input: { planId, activeNav, selectedPointId, selectedRouteId, deploymentChoices, mapOutputTitle, mapOutputZoom } });
  ...
  async function createMapOutputCanvas() {
    ...
    context.fillText(mapOutputTitle.trim() || "[請輸入地圖標題]道路警衛安全維護部署圖", canvas.width / 2, 88);
  ...
  async function exportDeploymentTable() { try { const fileName = `${planName || "DutyGrid"}-安全維護部署表.xlsx`; const path = await save({ defaultPath: fileName, filters: [{ name: "Excel 活頁簿", extensions: ["xlsx"] }] }); if (!path) { setMessage("已取消匯出安全維護部署表。 "); return; } const bytes = await invoke<number[]>("export_deployment_xlsx", { input: { planName, rows: deploymentRows.map((row) => ({ sequence: row.index, postType: row.postType, pointName: row.point.pointName, unit: row.units, policeCount: row.count, personnelText: row.names === "—" ? "" : row.names, radioText: row.radios === "—" ? "" : row.radios, equipmentText: row.equipment.join("、") })) } }); await invoke("save_exported_file", { path, bytes }); setMessage("已匯出保留範本下拉選單的安全維護部署表。 "); } catch (error) { setMessage(`無法匯出安全維護部署表：${String(error)}`); } }
  return (
    <main className={activeNav === "地圖輸出" ? "app-shell map-output-mode" : activeNav === "部署表" ? "app-shell preview-mode" : "app-shell"}>
      <header className="top-bar">
        ...
        <div className="top-actions">
          <button type="button" disabled={!planId} onClick={() => void saveWorkspace()}>儲存</button>
      ...
      <section className="workspace" aria-label="地圖工作區">
        <MapCanvas isDrawingRoute={isDrawingRoute} manualVertexColor={routeColor} manualVertices={manualVertices} onExportReady={(exporter) => { mapExporter.current = exporter; }} onMapClick={selectPointLocation} onPendingCancel={() => { setPendingCoordinate(null); setMessage("已取消放置暫存點位。"); }} onPointMoved={movePoint} onPointSelect={setSelectedPointId} onRouteVertex={addManualVertex} pendingColor={pointColor} pendingCoordinate={pendingCoordinate} personnelLabelPointId={selectedPersonnelLabelPointId} personnelLabels={personnelLabels} points={points} routeLines={[...routes.map((route) => ({ color: route.color, dashed: route.lineStyle === "dashed", coordinates: routeCoordinates(route) })), ...(manualVertices.length > 1 ? [{ color: routeColor, coordinates: manualVertices, dashed: routeLineStyle !== "solid", opacity: 0.45 }] : [])].filter((route) => route.coordinates.length > 1)} selectedPointId={selectedPointId} showPersonnelLabels={showPersonnelLabels} showPointLabels={showPointLabels} />
        {activeNav === "部署表" && <div className="map-preview-note">完整預覽對應「安全維護部署表」第一頁；正式匯出將自第 7 列開始填入。點選表格列可定位地圖點位。</div>}
        ...
        {activeNav === "路線" && <div className="map-line-style"><button className={routeLineStyle === "solid" ? "active" : ""} type="button" onClick={() => void updateRouteLineStyle("solid")}>實線</button><button className={routeLineStyle === "dashed" ? "active" : ""} type="button" onClick={() => void updateRouteLineStyle("dashed")}>虛線</button><button type="button" onClick={() => { setIsDrawingRoute((value) => !value); setMessage(isDrawingRoute ? "已完成繪製；請保存路線。" : "繪製模式：點擊地圖加入折點；空白鍵復原。 "); }}>{isDrawingRoute ? "完成繪製" : "開始繪製"}</button></div>}
      </section>
      <aside className={activeNav === "路線" ? "inspector route-inspector" : activeNav === "人力配置" ? "inspector personnel-inspector" : "inspector"} aria-label="詳細資料面板" onClick={(event) => { if (event.target === event.currentTarget) { setSelectedPointId(null); setSelectedRouteId(null); } }}>
        ...
        {activeNav !== "部署表" && activeNav !== "地圖輸出" && <p>{message}</p>}
