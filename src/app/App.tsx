...
export default function App() {
  ...
  function selectPointLocation(latitude: number, longitude: number) { if (!planId) { setMessage("[警告] 請先建立勤務計畫，再新增崗哨。"); return; } if (activeNav === "路線" && isDrawingRoute) { setManualVertices((current) => [...current, [longitude, latitude]]); setMessage("已加入路線節點；完成後請點選「完成繪製」。"); return; } if (activeNav === "路線") { setMessage(manualVertices.length >= 2 ? "請輸入路線名稱保存路線。" : "尚未點選「開始繪製」。"); return; } if (activeNav === "部署表") { setSelectedPointId(null); setSelectedPersonnelLabelPointId(null); setMessage("已取消部署表崗哨選取。 "); return; } if (activeNav !== "點位") { setMessage("請先從頂部導覽選擇「崗哨」，再於地圖新增勤務崗哨。"); return; } if (!isPlacingPoint) { setPointDrawReminder(true); setMessage("尚未點選「開始繪製」，無法繪製"); return; } if (selectedPointId) { setSelectedPointId(null); setMessage("已取消選取勤務崗哨。"); return; } setPendingCoordinate({ latitude, longitude, pointType: "duty", color: pointColor }); setMessage("已選取底圖位置，請填寫崗哨編號與名稱後保存。"); }
  function selectCustomPointLocation(x: number, y: number) { if (!planId) return; if (activeNav === "路線" && isDrawingRoute) { setManualVertices((current) => [...current, [x, y]]); setMessage("已加入 XY 路線節點；完成後請點選「完成繪製」。"); return; } if (activeNav !== "點位" || !isPlacingPoint) { setMessage(activeNav === "點位" ? "尚未點選「開始繪製」，無法繪製" : "請先選擇「崗哨」或「路線繪製」。"); return; } if (selectedPointId) { setSelectedPointId(null); return; } setPendingCoordinate({ latitude: y, longitude: x, coordinateX: x, coordinateY: y, pointType: "duty", color: pointColor }); setMessage("已選取底圖位置；請填寫崗哨編號與名稱後保存。"); }
  ...
  return (
    <main className={`app-shell ${activeNav === "地圖輸出" ? "map-output-mode" : activeNav === "部署表" ? "preview-mode" : ""} ${activePlan?.planMode === "custom_basemap" ? "custom-basemap-mode" : ""}`}>
      ...
      <aside className={activeNav === "路線" ? "inspector route-inspector" : activeNav === "人力配置" ? "inspector personnel-inspector" : "inspector"} aria-label="詳細資料面板" onClick={(event) => { const target = event.target as HTMLElement; if (activeNav === "點位" && !target.closest("button, input, select, textarea, label")) { setSelectedPointId(null); setEditingPointId(null); return; } if (event.target === event.currentTarget) { setSelectedPointId(null); setSelectedRouteId(null); } }}>
        ...
        {planId && activeNav === "勤務計畫" && <section className="plan-summary"><span>目前開啟</span><strong>{planName}</strong><p>請由頂部導覽選擇「崗哨」以新增、查看或刪除勤務崗哨。</p><button className="secondary-button" type="button" onClick={() => void exitPlan()}>退出勤務計畫</button></section>}
