...
export default function App() {
  ...
  async function updateRouteColor(color: string) { setRouteColor(color); const route = routes.find((item) => item.id === selectedRouteId); if (!route) return; try { await invoke("update_duty_route_color", { routeId: route.id, color }); setRoutes((current) => current.map((item) => item.id === route.id ? { ...item, color } : item)); setMessage(`已更新路線 ${route.routeName} 的顏色。`); } catch (error) { setMessage(`無法更新路線顏色：${String(error)}`); } }
