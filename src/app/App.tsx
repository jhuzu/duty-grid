...
export default function App() {
  ...
  function exitPlan() { setPlanId(null); setPlanName("尚未開啟勤務計畫"); setNewPlanName(""); setPoints([]); setRoutes([]); setPersonnelAssignments([]); setDeploymentChoices({}); setDeploymentEquipment({}); setPendingCoordinate(null); setSelectedPointId(null); setSelectedRouteId(null); setSelectedPersonnelLabelPointId(null); setEquipmentEditorPointId(null); setManualVertices([]); setIsDrawingRoute(false); setActiveNav("勤務計畫"); setMessage("請輸入勤務計畫名稱以建立新計畫。"); }
  ...
  return (
    <main className={activeNav === "地圖輸出" ? "app-shell map-output-mode" : activeNav === "部署表" ? "app-shell preview-mode" : "app-shell"}>
      ...
      <footer className="status-bar"><span>資料庫：尚未初始化　｜　路口參考：已隨 App 提供</span><span>© 1136023. All rights reserved</span></footer>
