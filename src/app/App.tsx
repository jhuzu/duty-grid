...
const navigation = ["勤務計畫", "路線", "點位", "人力配置", "部署表", "地圖輸出"];
const pointColors = [
...
export default function App() {
  ...
  useEffect(() => { if (activeNav === "點位" && !isPlacingPoint) setPointDrawReminder(true); }, [activeNav, isPlacingPoint]);
  useEffect(() => { if (activeNav === "勤務計畫") setMessage(planId ? "請由頂部導覽選擇「點位」以新增、查看或刪除勤務點位。" : "請輸入勤務計畫名稱以建立新計畫。"); }, [activeNav, planId]);
  ...
  async function savePointDetails(point: MapDutyPoint, form: HTMLFormElement) {
    ...
    const latitude = Number(values.get("latitude")); const longitude = Number(values.get("longitude"));
    if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) { setValidationError("請輸入有效的經緯度。"); return; }
    try {
      const updated = await invoke<MapDutyPoint>("update_duty_point", { pointId: point.id, input: { pointCode: String(values.get("pointCode") ?? ""), pointName: String(values.get("pointName") ?? ""), note: String(values.get("note") ?? ""), color: String(values.get("color") ?? ""), pointType: String(values.get("pointType") ?? ""), latitude, longitude } });
      setPoints((current) => current.map((item) => item.id === updated.id ? updated : item)); setValidationError(""); setMessage(`已更新點位 ${updated.pointCode}｜${updated.pointName}。`);
  ...
  function addManualDeploymentRow() { setManualDeploymentRows((rows) => [...rows, { id: crypto.randomUUID(), postType: "", location: "", unit: "", radioCode: "", coordinatorPhone: "", personnelIds: [], equipment: [] }]); }
  ...
  return (
    <main className={activeNav === "地圖輸出" ? "app-shell map-output-mode" : activeNav === "部署表" ? "app-shell preview-mode" : "app-shell"}>
      ...
      <aside className={activeNav === "路線" ? "inspector route-inspector" : activeNav === "人力配置" ? "inspector personnel-inspector" : "inspector"} aria-label="詳細資料面板" onClick={(event) => { const target = event.target as HTMLElement; if (activeNav === "點位" && !target.closest("button, input, select, textarea, label")) { setSelectedPointId(null); setEditingPointId(null); return; } if (event.target === event.currentTarget) { setSelectedPointId(null); setSelectedRouteId(null); } }}>
        ...
        {activeNav !== "部署表" && activeNav !== "地圖輸出" && activeNav !== "人力配置" && <p className={inspectorMessageClass}>{message}</p>}
