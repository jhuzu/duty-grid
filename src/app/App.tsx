...
export default function App() {
  ...
  function selectPointLocation(latitude: number, longitude: number) { if (!planId) { setMessage("請先建立勤務計畫，再新增點位。"); return; } if (isDrawingRoute) { setManualVertices((current) => [...current, [longitude, latitude]]); setMessage("已加入路線節點；按 Enter 完成，Backspace 復原。 "); return; } if (activeNav === "部署表") { setSelectedPointId(null); setSelectedPersonnelLabelPointId(null); setMessage("已取消部署表點位選取。 "); return; } if (activeNav !== "點位") { setMessage("請先從頂部導覽選擇「點位」，再於地圖新增勤務點位。"); return; } if (!isPlacingPoint) { setMessage("尚未點選「開始繪製」，無法繪製"); return; } if (selectedPointId) { setSelectedPointId(null); setMessage("已取消選取勤務點位。"); return; } setPendingCoordinate({ latitude, longitude }); setMessage("已選取地圖位置；請填寫點位編號與名稱後保存。"); }
  ...
  async function createPoint() { if (!planId || !pendingCoordinate) return; if (!pointCode.trim() || !pointName.trim()) { setValidationError("請輸入點位編號與位置名稱。"); return; } try { const point = await invoke<MapDutyPoint>("create_duty_point", { input: { planId, pointCode, pointName, color: pointType === "signal" ? "yellow" : pointColor, pointType, ...pendingCoordinate } }); setValidationError(""); setPoints((current) => [...current, point]); setPendingCoordinate(null); setIsPlacingPoint(false); setPointCode(/^\d+$/.test(point.pointCode) ? String(Number(point.pointCode) + 1) : ""); setPointName(""); setPointType("duty"); setMessage(`已保存${point.pointType === "signal" ? "號誌" : "勤務"}點位 ${point.pointCode}。`); } catch (error) { setMessage(String(error)); } }
  ...
  const mapRouteLines = activeNav === "部署表" && deploymentRoute
    ? [{ color: deploymentRoute.color, dashed: deploymentRoute.lineStyle === "dashed", coordinates: routeCoordinates(deploymentRoute) }].filter((route) => route.coordinates.length > 1)
    : [...routes.map((route) => ({ color: route.color, dashed: route.lineStyle === "dashed", coordinates: routeCoordinates(route) })), ...(manualVertices.length > 1 ? [{ color: routeColor, coordinates: manualVertices, dashed: routeLineStyle !== "solid", opacity: 0.45 }] : [])].filter((route) => route.coordinates.length > 1);
  ...
  return (
    <main className={activeNav === "地圖輸出" ? "app-shell map-output-mode" : activeNav === "部署表" ? "app-shell preview-mode" : "app-shell"}>
      ...
      <section className="workspace" aria-label="地圖工作區">
        ...
        {activeNav === "點位" && <div className="map-point-actions"><button className={isPlacingPoint ? "active" : ""} type="button" onClick={() => { setIsPlacingPoint((value) => !value); setMessage(isPlacingPoint ? "已取消新增點位繪製。" : "點位繪製模式：請點擊地圖放置點位。"); }}>{isPlacingPoint ? "取消繪製" : "開始繪製"}</button></div>}
      </section>
