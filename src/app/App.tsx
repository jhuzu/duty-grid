...
export default function App() {
  ...
  function selectPointLocation(latitude: number, longitude: number) { if (!planId) { setMessage("請先建立勤務計畫，再新增點位。"); return; } if (isDrawingRoute) { setManualVertices((current) => [...current, [longitude, latitude]]); setMessage("已加入路線節點；按 Enter 完成，Backspace 復原。 "); return; } if (activeNav === "部署表") { setSelectedPointId(null); setSelectedPersonnelLabelPointId(null); setMessage("已取消部署表點位選取。 "); return; } if (activeNav !== "點位") { setMessage("請先從頂部導覽選擇「點位」，再於地圖新增勤務點位。"); return; } if (!isPlacingPoint) { setPointDrawReminder(true); setMessage("尚未點選「開始繪製」，無法繪製"); return; } if (selectedPointId) { setSelectedPointId(null); setMessage("已取消選取勤務點位。"); return; } setPendingCoordinate({ latitude, longitude }); setMessage("已選取地圖位置；請填寫點位編號與名稱後保存。"); }
