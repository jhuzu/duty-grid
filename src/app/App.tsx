  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace" aria-label="地圖工作區">
        <MapCanvas isDrawingRoute={isDrawingRoute} manualVertexColor={routeColor} manualVertices={manualVertices} onMapClick={selectPointLocation} onPendingCancel={() => { setPendingCoordinate(null); setMessage("已取消放置暫存點位。"); }} onPointMoved={movePoint} onPointSelect={setSelectedPointId} onRouteVertex={addManualVertex} pendingColor={pointColor} pendingCoordinate={pendingCoordinate} points={points} routeLines={[...routes.map((route) => ({ color: route.color, dashed: route.lineStyle === "dashed", coordinates: routeCoordinates(route) })), ...(manualVertices.length > 1 ? [{ color: routeColor, coordinates: manualVertices, dashed: routeLineStyle !== "solid", opacity: 0.45 }] : [])].filter((route) => route.coordinates.length > 1)} selectedPointId={selectedPointId} />
      ...
      <aside className={activeNav === "路線" ? "inspector route-inspector" : "inspector"} aria-label="詳細資料面板" onClick={(event) => { if (event.target === event.currentTarget) { setSelectedPointId(null); setSelectedRouteId(null); } }}>
