...
export type MapDutyPoint = { id: string; pointCode: string; pointName: string; note?: string | null; color: string; pointType: "duty" | "hollow" | "signal"; latitude: number; longitude: number; coordinateX?: number | null; coordinateY?: number | null };
export function MapCanvas({ bearing = 0, dimmedPointIds = [], fitToData = false, focusCenter, focusCoordinates, interactive = true, isDrawingRoute, manualVertexColor, manualVertices, onBearingChange, onExportReady, onMapClick, onMapPointerMove, onPendingCancel, onPointMoved, onPointRename, onPointSelect, onRouteVertex, pendingColor, pendingCoordinate, pendingPointType = "duty", personnelLabelPointId, personnelLabels, points, routeLines, selectedPointId, showNavigation = true, showPersonnelLabels, showPointLabels, zoomAdjustment = 0 }: { bearing?: number; dimmedPointIds?: string[]; fitToData?: boolean; focusCenter?: [number, number]; focusCoordinates?: [number, number][]; interactive?: boolean; isDrawingRoute: boolean; manualVertexColor: string; manualVertices: [number, number][]; onBearingChange?: (bearing: number) => void; onExportReady: (exporter: () => string | null) => void; onMapClick: (latitude: number, longitude: number) => void; onMapPointerMove?: (latitude: number, longitude: number) => void; onPendingCancel: () => void; onPointMoved: (point: MapDutyPoint, latitude: number, longitude: number) => void; onPointRename: (point: MapDutyPoint) => void; onPointSelect: (pointId: string) => void; onRouteVertex: (latitude: number, longitude: number) => void; pendingColor: string; pendingCoordinate: { latitude: number; longitude: number } | null; pendingPointType?: MapDutyPoint["pointType"]; personnelLabelPointId: string | null; personnelLabels: Record<string, string[]>; points: MapDutyPoint[]; routeLines: { color: string; coordinates: [number, number][]; dashed?: boolean; arrow?: boolean; opacity?: number }[]; selectedPointId: string | null; showNavigation?: boolean; showPersonnelLabels: boolean; showPointLabels: boolean; zoomAdjustment?: number }) {
  ...
  const onPointRenameRef = useRef(onPointRename);
  ...
  useEffect(() => { onPointMovedRef.current = onPointMoved; }, [onPointMoved]);
  useEffect(() => { onPointRenameRef.current = onPointRename; }, [onPointRename]);
  ...
  useEffect(() => {
    ...
    map.current.fitBounds([[west, south], [east, north]], { duration: 350, maxZoom: 16, padding: 80 });
  }, [focusCoordinatesKey]);
  useEffect(() => { const focusRoute = (event: Event) => { const coordinates = (event as CustomEvent<[number, number][]>).detail?.filter(([longitude, latitude]) => Number.isFinite(longitude) && Number.isFinite(latitude) && Math.abs(longitude) <= 180 && Math.abs(latitude) <= 90) ?? []; if (!coordinates.length || !map.current) return; const longitude = coordinates.reduce((sum, [value]) => sum + value, 0) / coordinates.length; const latitude = coordinates.reduce((sum, [, value]) => sum + value, 0) / coordinates.length; map.current.easeTo({ center: [longitude, latitude], duration: 450 }); }; window.addEventListener("dutygrid:focus-route", focusRoute); return () => window.removeEventListener("dutygrid:focus-route", focusRoute); }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    map.current.on("error", (event) => {
      if (!hasFallback.current && (event as { sourceId?: string }).sourceId === "basemap") {
  ...
  useEffect(() => {
    if (!container.current || !map.current) return;
    if (routeLines.some((route) => route.coordinates.some(([longitude, latitude]) => !Number.isFinite(latitude) || !Number.isFinite(longitude) || Math.abs(latitude) > 90 || Math.abs(longitude) > 180))) return;
